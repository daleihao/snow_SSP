...
for model_t = 1:7
    ...
    for model_i =1:model_t
        ...
        total_BCs_1995(:,:,model_i) =  nanmean(wet_BCs(:,:,month_i_all),3);
        total_dusts_1995(:,:,model_i) = nanmean(wet_dusts(:,:,month_i_all),3);
        ...
        total_BCs_2100(:,:,model_i) = nanmean(wet_BCs(:,:,month_i_all),3);
        total_dusts_2100(:,:,model_i) = nanmean(wet_dusts(:,:,month_i_all),3);
    end
    ...
    print(gcf, '-dtiff', '-r300', ['../../figure/wet_' ModelNames{model_i} '_aero_deposit_pattern_trend' '.tif'])
  close all
end
%% output
