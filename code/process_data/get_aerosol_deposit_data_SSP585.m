...
for model_i =[1 7]
    if model_i == 1
        file_name = ['../data/aerosol_deposit_data/SSP585/*BSSP585cmip6*.nc'];
        filenames = dir(file_name);
        file_name = ['../data/aerosol_deposit_data/SSP585/' filenames(1).name];
    ...
    elseif model_i == 7
        file_name = ['../data/aerosol_deposit_data/SSP585/NorESM2-LM/wetbc_*.nc'];
        filenames = dir(file_name);
        filenum = size(filenames,1);
        wet_BCs = [];
        for index_i = 1:filenum
            file_name = ['../data/aerosol_deposit_data/SSP585/NorESM2-LM/' filenames(index_i).name];
            tmp = ncread(file_name,'wetbc')*1e11;
            wet_BCs = cat(3,wet_BCs,tmp);
        end
        
        file_name = ['../data/aerosol_deposit_data/SSP585/NorESM2-LM/drybc_*.nc'];
        filenames = dir(file_name);
        filenum = size(filenames,1);
        dry_BCs = [];
        for index_i = 1:filenum
        ...
        file_name = ['../data/aerosol_deposit_data/SSP585/NorESM2-LM/wetdust_*.nc'];
        filenames = dir(file_name);
        filenum = size(filenames,1);
        wet_dusts = [];
        for index_i = 1:filenum
        ...
        file_name = ['../data/aerosol_deposit_data/SSP585/NorESM2-LM/drydust_*.nc'];
        filenames = dir(file_name);
        filenum = size(filenames,1);
        dry_dusts = [];
        for index_i = 1:filenum
        ...
        file_name = ['../data/aerosol_deposit_data/SSP585/wetbc_AERmon_' ModelNames{model_i} '_*.nc'];
        filenames = dir(file_name);
        file_name = ['../data/aerosol_deposit_data/SSP585/' filenames(1).name];
        wet_BCs = ncread(file_name,'wetbc')*1e11;
        file_name = ['../data/aerosol_deposit_data/SSP585/drybc_AERmon_'  ModelNames{model_i}  '*.nc'];
        filenames = dir(file_name);
        file_name = ['../data/aerosol_deposit_data/SSP585/' filenames(1).name];
        dry_BCs = ncread(file_name,'drybc')*1e11;
        
        file_name = ['../data/aerosol_deposit_data/SSP585/wetdust_AERmon_'  ModelNames{model_i}  '*.nc'];
        filenames = dir(file_name);
        file_name = ['../data/aerosol_deposit_data/SSP585/' filenames(1).name];
        wet_dusts = ncread(file_name,'wetdust')*1e8;
        
        file_name = ['../data/aerosol_deposit_data/SSP585/drydust_AERmon_'  ModelNames{model_i}  '*.nc'];
        filenames = dir(file_name);
        file_name = ['../data/aerosol_deposit_data/SSP585/' filenames(1).name];
    ...
    save(['585_aer_' ModelNames{model_i} '.mat'],'wet_BCs', 'dry_BCs', 'wet_dusts', 'dry_dusts');
