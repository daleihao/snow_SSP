...
colormap(colors_1)
plot_global_map_log(lats, lons, mean_BCs_1995, -2,1,"1995~2014");
...
colormap(colors_1)
plot_global_map_log(lats, lons, mean_BCs_2100, -2,1,"2081~2100");
...
plot_global_map_trend_log(lats, lons, mean_BCs_2100 - mean_BCs_1995,p_val_BCs,Slope_BCs,-0.3,0.3,"Difference",0);
...
if(sig<0.05)
    text(2052,0.68,'p<0.05,','fontweight','bold','fontsize',10,'color',colors(1,:))
else
    text(2052,0.68,['p=' num2str(sig,'%4.2f') ','],'fontweight','bold','fontsize',10,'color',colors(1,:))
end
...
[taub tau h sig Z S sigma sen n senplot CIlower CIupper D Dall C3 nsigma] = ktaub([Years mean_BC_time_series], 0.05, 0);
if(sig<0.05)
    text(2076,0.68,'p<0.05','fontweight','bold','fontsize',10,'color',colors(2,:))
else
    text(2076,0.68,['p=' num2str(sig,'%4.2f')],'fontweight','bold','fontsize',10,'color',colors(2,:))
end
...
colormap(colors_1)
plot_global_map_log(lats, lons, mean_dusts_1995, -2,1,"");
...
colormap(colors_1)
plot_global_map_log(lats, lons, mean_dusts_2100, -2,1,"");
text(-1.3,1.1,'i','fontweight','bold','fontsize',12)

ax7 = subplot('position', [0.01 + 0.21*(3-1) 0.28 0.21 0.2]);
colormap(colors_1)
plot_global_map_trend_log(lats, lons, mean_dusts_2100 - mean_dusts_1995,p_val_dusts,Slope_dusts,-0.1,0.1,"",1);
...
ylabel('\mug m^{-2} s^{-1}')
box on
text(1980,0.19,'k','fontweight','bold','fontsize',12)
...
[taub tau h sig Z S sigma sen n senplot CIlower CIupper D Dall C3 nsigma] = ktaub([Years mean_BC_time_series], 0.05, 0);
if(sig<0.05)
    text(2052,0.82,'p<0.05,','fontweight','bold','fontsize',10,'color',colors(1,:))
else
    text(2052,0.82,['p=' num2str(sig,'%4.2f') ','],'fontweight','bold','fontsize',10,'color',colors(1,:))
end
mean_BC_time_series = mean(squeeze(BCs_time_series_all(:,(1995-1950+1):end,2)'),2);
min_BC_time_series = mean_BC_time_series+std(squeeze(BCs_time_series_all(:,(1995-1950+1):end,2)'),1,2)/2;
max_BC_time_series = mean_BC_time_series-std(squeeze(BCs_time_series_all(:,(1995-1950+1):end,2)'),1,2)/2;
...
[taub tau h sig Z S sigma sen n senplot CIlower CIupper D Dall C3 nsigma] = ktaub([Years mean_BC_time_series], 0.05, 0);
if(sig<0.05)
    text(2076,0.82,'p<0.05','fontweight','bold','fontsize',10,'color',colors(2,:))
else
    text(2076,0.82,['p=' num2str(sig,'%4.2f')],'fontweight','bold','fontsize',10,'color',colors(2,:))
end
xlim([1995 2100])
ylim([0 0.9])
box on
...
box on
ylabel('\mug m^{-2} s^{-1}')
text(1980,0.21,'n','fontweight','bold','fontsize',12)
yyaxis right
yticks([])
ylabel('SSP585','fontweight','bold');
set(gca,'Ycolor','K');
