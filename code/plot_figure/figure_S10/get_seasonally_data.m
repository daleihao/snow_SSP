...
for month_i = 1:12
    for  case_i = 1:size(casenames,2)
        ...
        SS_BC_means = nan(720,140,year_num);
        SS_dust_means = nan(720,140,year_num);
        
        SWE_means =  nan(720,140,year_num);
        FSNO_means = nan(720,140,year_num);
        
        SNOW_means = nan(720,140,year_num);
        RAIN_means = nan(720,140,year_num);
        ...
        if case_i<=4
            for year_i = 2015:2100
                ...
                SS_BC_means(:,:,year_index) = mean(SNOBCMSLs(:,:,month_i_all),3,'omitnan');
                SS_dust_means(:,:,year_index) = mean(SNODSTMSLs(:,:,month_i_all),3,'omitnan');
                ...
                SNORDSL_means(:,:,year_index) = mean(RAINs(:,:,month_i_all),3,'omitnan');
            end
        else
        ...
        save([ 'Month_' num2str(month_i) '_mean_' casename '.mat'],...
