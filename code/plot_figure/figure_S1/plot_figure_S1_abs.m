...
Slope_BC_126 = Slope_BCs*100;
Slope_Dust_126 = Slope_dusts*100;

Slope_BC_126(p_val_BCs>=0.05 | nanmean(total_SWEs_all,3)<=5) = nan;
Slope_Dust_126(p_val_dusts>=0.05 | nanmean(total_SWEs_all,3)<=5) = nan;

load('585_p_values.mat');
load('585_total_SWE_all.mat');
load('585_total_BC_all.mat');

Slope_BC_585 = Slope_BCs*100;
Slope_Dust_585 = Slope_dusts*100;
...
set(gcf,'unit','normalized','position',[0.1,0.05,0.42,0.8]);
set(gca, 'Position', [0 0 1 1])

ax1 = subplot('position', [0.06 + 0.421*(1-1) 0.575 0.43 0.38]);
colormap(colors_1)
...
hcb.Title.String = '10^{-2} ng m^{-2} s^{-1} yr^{-1}';
...
ax2 = subplot('position', [0.06 + 0.46*(2-1) 0.575 0.43 0.38]);
colormap(colors_1)
...
%%SWE
ax5 = subplot('position', [0.06 + 0.421*(1-1) 0.10 0.43 0.38]);
colormap(colors_1)
plot_global_map(lats, lons, Slope_Dust_126, -0.1, 0.1,"");
...
hcb = colorbar;
hcb.Location = 'southoutside';
hcb.Title.String = '10^{-2} \mug m^{-2} s^{-1} yr^{-1}';
...
ax6 = subplot('position', [0.06 + 0.46*(2-1) 0.10 0.43 0.38]);
colormap(colors_1)
plot_global_map(lats, lons, Slope_Dust_585, -0.1, 0.1,"");
...
%% output
print(gcf, '-dtiff', '-r300', '../../figures/figure_S1_abs_r.tif')
