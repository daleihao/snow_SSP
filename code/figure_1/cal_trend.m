...
mean_swes_1995 = nanmean(total_SWEs_all,3);
...
for row_i = 1:size(total_SWEs_all, 1)
    for col_i = 1:size(total_SWEs_all, 2)
        ...
        if(sum(isnan(BCs_tmp))<1 & sum(BCs_tmp)>0 & mean_swes_1995(row_i,col_i)>swe_threshold & lats(row_i,col_i)>23.5)
            [taub tau h sig Z S sigma sen n senplot CIlower CIupper D Dall C3 nsigma] = ktaub([Years' BCs_tmp(21:end)], 0.05, 0);
...
for row_i = 1:size(total_dusts_all, 1)
    for col_i = 1:size(total_dusts_all, 2)
        ...
        if(sum(isnan(BCs_tmp))<1 & sum(BCs_tmp)>0 & mean_swes_1995(row_i,col_i)>swe_threshold & lats(row_i,col_i)>23.5)
            [taub tau h sig Z S sigma sen n senplot CIlower CIupper D Dall C3 nsigma] = ktaub([Years' dusts_tmp(21:end)], 0.05, 0);
